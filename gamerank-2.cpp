...
auto process_match( char result, Status current_status ) -> Status;
auto process_win( Status current_status )                -> Status;
auto process_loss( Status current_status )               -> Status;
...
auto process_matches( std::string_view matches ) -> int
{
   ...
   for ( auto outcome : matches )
   {
      status = process_match( outcome, status );
   }
...
auto process_match( char result, Status current_status ) -> Status
{
   ...
   {
      case 'W':
         return process_win( current_status );

      case 'L':
         return process_loss( current_status );
